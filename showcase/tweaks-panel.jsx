...
//
// Standalone mode: when window.__SNAP_STANDALONE__ is set, the panel auto-opens
// and a small fixed "TWEAKS" toggle is rendered so the user can hide/show it
// without a host iframe driving __activate_edit_mode.
//
// ─────────────────────────────────────────────────────────────────────────────
...
const __TWEAKS_STYLE = `
  .twk-panel{position:fixed;right:16px;bottom:16px;z-index:2147483646;width:280px;
    ...
    box-shadow:0 1px 0 rgba(255,255,255,.5) inset,0 12px 40px rgba(0,0,0,.35);
  ...
  .twk-trigger{position:fixed;right:16px;bottom:16px;z-index:2147483645;
    background:#000;color:#CEFF00;border:1px solid #27272a;padding:10px 14px;
    font:600 11px/1 'JetBrains Mono', ui-monospace, monospace;letter-spacing:.18em;
    cursor:pointer;border-radius:0;text-transform:uppercase}
  .twk-trigger:hover{background:#0a0a0a}
`;
...
function useTweaks(defaults) {
  ...
  const setTweak = React.useCallback((keyOrEdits, val) => {
    ...
    } catch (_) { /* standalone: parent same-origin no-op */ }
  }, []);
...
function TweaksPanel({ title = 'Tweaks', children }) {
  const standalone = !!window.__SNAP_STANDALONE__;
  ...
  if (!open) {
    ...
    return (
      <>
        ...
        <button className="twk-trigger" onClick={() => setOpen(true)} aria-label="Open tweaks">
          ▸ TWEAKS
        </button>
      </>
    );
